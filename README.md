...
This builds the image and checks: host home is readable but not writable (even with sudo), workspace is writable, SSH keys are hidden and git push fails, GitHub CLI credentials are hidden, no-push rules are in the entrypoint, GPU/CUDA work, sudo works, uv/uvx, Claude Code, ttyd, and xclip are available, terminal env is forwarded, default settings are baked into the image, PulseAudio socket is mounted, clipboard access works, `apt-get install` works, container naming works, settings merge runs correctly, and the agent can commit inside a worktree.
